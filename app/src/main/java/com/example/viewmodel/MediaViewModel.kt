...
class MediaViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _aiRecommendations = MutableStateFlow<List<com.example.model.AiRecommendationResult>>(emptyList())
    val aiRecommendations: StateFlow<List<com.example.model.AiRecommendationResult>> = _aiRecommendations.asStateFlow()
    ...
    private val _chatHistory = MutableStateFlow<List<com.example.model.ChatMessage>>(emptyList())
    val chatHistory: StateFlow<List<com.example.model.ChatMessage>> = _chatHistory.asStateFlow()
    ...
    fun sendAiChatMessage(userMessage: String) {
        ...
        viewModelScope.launch {
            ...
            _chatHistory.value = _chatHistory.value + com.example.model.ChatMessage(role = "user", text = userMessage)
            ...
            try {
                ...
                _chatHistory.value = _chatHistory.value + com.example.model.ChatMessage(
                    role = "model",
    ...
    private val _topAnime = MutableStateFlow<List<com.example.model.JikanAnime>>(emptyList())
    val topAnime: StateFlow<List<com.example.model.JikanAnime>> = _topAnime.asStateFlow()
            ...
            if (proxyUrl.isNotEmpty() && proxyUrl != "YOUR_CLOUDFLARE_WORKER_URL" && query.length >= 3) {
                try {
                    ...
                    val req = okhttp3.Request.Builder()
                        ...
                        .post(reqBody)
                        .build()
                    val response = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) { com.example.network.NetworkModule.okHttpClient.newCall(req).execute() }
